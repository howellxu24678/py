...
DailyRollingFileAppender::init(DailyRollingFileSchedule sch)
{
    ...
    helpers::getLogLog().debug(LOG4CPLUS_TEXT("now2: ") + now.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
    scheduledFilename = getFilename(now);
    nextRolloverTime = calculateNextRolloverTime(now);
    helpers::getLogLog().debug(LOG4CPLUS_TEXT("nextRolloverTime: ") + nextRolloverTime.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
}
...
DailyRollingFileAppender::append(const spi::InternalLoggingEvent& event)
{
    if(event.getTimestamp() >= nextRolloverTime) {
      helpers::getLogLog().debug(LOG4CPLUS_TEXT("event.getTimestamp(): ") + event.getTimestamp().getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
      helpers::getLogLog().debug(LOG4CPLUS_TEXT("nextRolloverTime: ") + nextRolloverTime.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
        rollover(true);
    }
...
DailyRollingFileAppender::rollover(bool alreadyLocked)
{
    ...
    helpers::getLogLog().debug(LOG4CPLUS_TEXT("rollover now: ") + now.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
    if (now >= nextRolloverTime)
    {
        helpers::getLogLog().debug(LOG4CPLUS_TEXT("rollover nextRolloverTime1: ") + nextRolloverTime.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
        scheduledFilename = getFilename(now);
        nextRolloverTime = calculateNextRolloverTime(now);
        helpers::getLogLog().debug(LOG4CPLUS_TEXT("rollover nextRolloverTime2: ") + nextRolloverTime.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
    }
}
...
DailyRollingFileAppender::calculateNextRolloverTime(const Time& t) const
{
  ...
  next_t.usec(0);
  struct tm time;
  next_t.localtime(&time);

  helpers::getLogLog().debug(LOG4CPLUS_TEXT("now in: ") + next_t.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
  ...
  helpers::getLogLog().debug(LOG4CPLUS_TEXT("now out: ") + next_t.getFormattedTime(LOG4CPLUS_TEXT("%Y-%m-%d-%H-%M-%S"),false));
  //return adjust_for_time_zone (next_t, local_time_offset (next_t));
  return next_t;

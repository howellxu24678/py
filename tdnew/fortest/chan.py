...
class Pen(Base):
    def __init__(self, **kwargs):
        ...
        self.loc = kwargs['loc'] if 'loc' in kwargs else 'un'
        self.value = kwargs['value'] if 'value' in kwargs else 'un'
        self.shape = kwargs['shape'] if 'shape' in kwargs else 'un'
...
class Seq(Base):
    def __init__(self, **kwargs):
        super(Pen, self).__init__()
        self.bloc = kwargs['bloc'] if 'bloc' in kwargs else 'un'
        self.eloc = kwargs['eloc'] if 'eloc' in kwargs else 'un'
        self.bvalue = kwargs['bvalue'] if 'bvalue' in kwargs else 'un'
        self.evalue = kwargs['evalue'] if 'evalue' in kwargs else 'un'
        self.high = kwargs['high'] if 'high' in kwargs else 'un'
        self.low = kwargs['low'] if 'low' in kwargs else 'un'


k = Kline(high = 1.2, low = 2.3, shape = 'u')
...
class Chan(object):
    ...
    def _procContain(df, isUp, newkline):
        if df.shape[0] < 1 or not Chan.isContain(df.iloc[-1], newkline):
            Chan._setValue(df, newkline)
        else:
            ...
            return Chan._procContain(df, isUp, newkline)
    ...
    def procPen(self):
        ...
        if self._pen.shape[0] < 2:
            ...
            if abs(dhLoc - dlLoc) >= 4:
                if dhLoc > dlLoc:
                    self._pen.loc[self._pen.shape[0]] = {'loc': dlLoc, 'shape': dl.iloc[0]['shape'],
                                                         'value': dl.iloc[0]['low']}
                    self._pen.loc[self._pen.shape[0]] = {'loc': dhLoc, 'shape': dh.iloc[0]['shape'],
                                                         'value': dh.iloc[0]['high']}
                else:
                    self._pen.loc[self._pen.shape[0]] = {'loc': dhLoc, 'shape': dh.iloc[0]['shape'],
                                                         'value': dh.iloc[0]['high']}
                    self._pen.loc[self._pen.shape[0]] = {'loc': dlLoc, 'shape': dl.iloc[0]['shape'],
